...
export const CustomerCitiesView: React.FC<CustomerCitiesViewProps> = ({
  ...
  // Spots dynamically derived from vendor businesses
  const spots: (CitySpotCard & { city?: string; category?: string })[] = useMemo(() => {
    return displayBusinesses.map((biz) => {
      ...
      const neighborhood = addrParts.length > 0 ? addrParts.join(', ') : 'United States';

      return {
        id: `city-spot-${biz.id}`,
        businessId: biz.id,
        name: biz.coreDetails?.businessName || 'Untitled Business',
        categoryBadge: ((biz.coreDetails?.category || 'BEAUTY').split(' ')[0]).toUpperCase() as any,
  ...
  const filteredSpots = useMemo(() => {
    return spots.filter((spot) => {
      if (activeCity !== 'All Cities' && spot.city?.toLowerCase() !== activeCity.toLowerCase()) {
        return false;
      }
      // Category filter
      if (activeCategoryPill !== 'All') {
        const fullCat = (spot.category || '').toLowerCase();
        const cat = String(spot.categoryBadge).toLowerCase();
        const pill = activeCategoryPill.toLowerCase();
        const match =
          fullCat === pill ||
          fullCat.includes(pill) ||
          pill.includes(fullCat) ||
          cat === pill ||
          pill.includes(cat);
        if (!match) {
          return false;
  ...
  return (
    <div className="w-full bg-white py-10 animate-in fade-in duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        {/* 4-Column Grid of Spot Cards */}
        {filteredSpots.length === 0 ? (
          <div className="p-12 text-center bg-slate-50 rounded-2xl border border-slate-200">
            <p className="text-sm font-bold text-slate-800">No businesses found in {activeCity}</p>
            <p className="text-xs text-slate-500 mt-1">Try selecting another city or resetting your category filter.</p>
          </div>
        ) : (
